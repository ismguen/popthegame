...
window.showMap = window.showMenu;
...
function initGrid() {
    ...
    let features = {
        rocks: 0, webs: 0, ice: 0, wind: 0, mirrors: 0, portals: false,
        elements: ['water']
    };

    if (level >= 2) features.rocks = 0.05 + (level * 0.01);
    if (level >= 3) features.webs = 0.1;
    if (level >= 4) features.elements.push('fire');
    if (level >= 5) features.ice = 0.1;
    if (level >= 6) features.elements.push('lightning');
    if (level >= 7) features.elements.push('acid');
    if (level >= 8) features.wind = 0.05;
    if (level >= 9) features.mirrors = 0.05;
...
// Initialize grid for the background menu and start engine
initGrid();
updateHUD();
window.generateMap();
loop();
